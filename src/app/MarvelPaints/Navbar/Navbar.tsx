...
const Navbar = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-gray-300 dark:bg-gray-400 shadow-md transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          {/* Logo */}
          <div className="flex-shrink-0">
            <Link href="/MarvelPaints">
              <Image
          ...
          <div className="hidden md:flex md:items-center md:space-x-10">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-gray-800 dark:text-black hover:text-blue-600 dark:hover:text-blue-400 font-medium transition-colors duration-300"
              >
                {link.label}
              </Link>
            ))}
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="md:hidden bg-white dark:bg-gray-400 shadow-md transition-all duration-500">
          <div className="px-4 pt-3 pb-4 space-y-2">
            {navLinks.map((link) => (
              <Link
                ...
                className="block px-3 py-2 rounded-md text-base font-medium dark:text-black hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-300"
              >
                {link.label}
              </Link>
            ))}
